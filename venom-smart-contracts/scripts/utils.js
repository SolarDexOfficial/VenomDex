...
const displayAccount = async (contract) => {
  return (
    `Account.${contract.name}${
      contract.index !== undefined ? '#' + contract.index : ''
    }` + `(address="${contract.address}" balance=${await getBalance(contract)})`
  );
};

const getBalance = async (contract) => {
  return locklift.utils
    .convertCrystal(await locklift.ton.getBalance(contract.address), 'ton')
    .toNumber();
};
...
const displayTx = (_tx) => {
  if (locklift.tracing) {
    ...
    console.log(
      `txId: ${
        _tx.transaction.id.hash ? _tx.transaction.id.hash : _tx.transaction.id
      }`,
    );
  }
};
...
class Migration {
  ...
  getAddress(alias) {
    if (this.migration_log[alias] !== undefined) {
      ...
      return new Address(this.migration_log[alias]);
    } else {
      throw new Error(`Contract ${alias} not found in the migration`);
    }
  }
  ...
  async balancesLastDiff() {
    ...
    for (let alias in this.migration_log) {
      const start =
        this.balance_history[this.balance_history.length - 2][alias];
  ...
  async logGas() {
    ...
    if (diff) {
      ...
      for (let alias in diff) {
        logger.log(
          `${alias}: ${diff[alias].gt(0) ? '+' : ''}${diff[alias].toFixed(
            9,
          )} EVER`,
        );
      }
...
function logExpectedDeposit(expected, tokens) {
  ...
  if (expected.step_1_lp_reward.isZero()) {
    ...
    logger.log(`    Step 1: `);
    logger.log(
      `        Left deposit = ${expected.step_1_left_deposit
        .shiftedBy(-left_decimals)
        .toFixed(left_decimals)}`,
    );
    logger.log(
      `        Right deposit = ${expected.step_1_right_deposit
        .shiftedBy(-right_decimals)
        .toFixed(right_decimals)}`,
    );
    logger.log(
      `        LP reward = ${expected.step_1_lp_reward
        .shiftedBy(-Constants.LP_DECIMALS)
        .toFixed(Constants.LP_DECIMALS)}`,
    );
  }
  if (expected.step_2_left_to_right) {
    logger.log(`    Step 2: `);
    logger.log(
      `        Left amount for change = ${expected.step_2_spent
        .shiftedBy(-left_decimals)
        .toFixed(left_decimals)}`,
    );
    logger.log(
      `        Left fee = ${expected.step_2_fee
        .shiftedBy(-left_decimals)
        .toFixed(left_decimals)}`,
    );
    logger.log(
      `        Right received amount = ${expected.step_2_received
        .shiftedBy(-right_decimals)
        .toFixed(right_decimals)}`,
    );
    ...
    logger.log(`    Step 2: `);
    logger.log(
      `        Right amount for change = ${expected.step_2_spent
        .shiftedBy(-right_decimals)
        .toFixed(right_decimals)}`,
    );
    logger.log(
      `        Right fee = ${expected.step_2_fee
        .shiftedBy(-right_decimals)
        .toFixed(right_decimals)}`,
    );
    logger.log(
      `        Left received amount = ${expected.step_2_received
        .shiftedBy(-left_decimals)
        .toFixed(left_decimals)}`,
    );
  } else {
  ...
  if (expected.step_3_lp_reward.isZero()) {
    ...
    logger.log(`    Step 3: `);
    logger.log(
      `        Left deposit = ${expected.step_3_left_deposit
        .shiftedBy(-left_decimals)
        .toFixed(left_decimals)}`,
    );
    logger.log(
      `        Right deposit = ${expected.step_3_right_deposit
        .shiftedBy(-right_decimals)
        .toFixed(right_decimals)}`,
    );
    logger.log(
      `        LP reward = ${expected.step_3_lp_reward
        .shiftedBy(-Constants.LP_DECIMALS)
        .toFixed(Constants.LP_DECIMALS)}`,
    );
  }
...
async function expectedDepositLiquidity(
  pairAddress,
  contractName,
  tokens,
  amounts,
...
async function expectedDepositLiquidityOneCoin(
  poolAddress,
  tokens,
  amount,
  spent_token_root,
