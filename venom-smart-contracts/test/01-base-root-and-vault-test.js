...
describe('Check for correct deployment', async function () {
  ...
  before('Load contracts', async function () {
    ...
    DexPlatform = await locklift.factory.getContract(
      'DexPlatform',
      'precompiled',
    );
    DexAccount = await locklift.factory.getContract(
  ...
  describe('Check DexRoot', async function () {
    it('Check DexRoot deployed', async function () {
      expect(
        (await locklift.ton.getAccountType(dexRoot.address)).acc_type,
      ).to.equal(1, 'DexRoot Account not Active');
    });
    it('Check Platform code is installed', async function () {
      expect(await dexRoot.call({ method: 'platform_code' })).to.equal(
        DexPlatform.code,
        'Wrong platform code in DexRoot',
      );
    });
    it('Check Account code is installed', async function () {
      expect(await dexRoot.call({ method: 'getAccountCode' })).to.equal(
        DexAccount.code,
    ...
    it('Check Pair code is installed', async function () {
      expect(
        await dexRoot.call({ method: 'getPairCode', params: { pool_type: 1 } }),
      ).to.equal(DexPair.code, 'Wrong Pair code in DexRoot');
    });
    it('Check Vault address', async function () {
      expect(await dexRoot.call({ method: 'getVault', params: {} })).to.equal(
        dexVault.address,
        'Wrong DexVault address in DexRoot',
      );
    });
    it('Check is Dex Active', async function () {
      expect(await dexRoot.call({ method: 'isActive', params: {} })).to.equal(
        true,
        'DexRoot is not Active',
      );
    });
  });
  describe('Check DexVault', async function () {
    it('Check DexVault deployed', async function () {
      expect(
        (await locklift.ton.getAccountType(dexVault.address)).acc_type,
      ).to.equal(1, 'DexVault Account not Active');
    });
    it('Check Platform code is installed', async function () {
      expect(await dexVault.call({ method: 'platform_code' })).to.equal(
        DexPlatform.code,
        'Wrong platform code in DexVault',
      );
    });
    it('Check Root address', async function () {
      expect(await dexVault.call({ method: 'getRoot' })).to.equal(
        dexRoot.address,
        'Wrong DexRoot address in DexVault',
      );
    });
    it.skip('Check TokenFactory address', async function () {
      expect(await dexVault.call({ method: 'getTokenFactory' })).to.equal(
        tokenFactory.address,
        'Wrong TokenFactory address in DexVault',
      );
    });
    it.skip('Check DexVaultLpTokenPending code is installed', async function () {
      expect(await dexVault.call({ method: 'getLpTokenPendingCode' })).to.equal(
        DexVaultLpTokenPending.code,
        'Wrong DexVaultLpTokenPending code in DexVault',
      );
    });
  });
});
