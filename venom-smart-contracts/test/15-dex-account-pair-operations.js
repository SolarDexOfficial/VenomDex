...
async function dexAccountBalances(account) {
  ...
  let walletFoo = '0';
  await FooWallet2.call({ method: 'balance', params: {} })
    .then((n) => {
      walletFoo = new BigNumber(n)
        .shiftedBy(-Constants.tokens.foo.decimals)
        .toString();
    })
    .catch((e) => {
  ...
  let walletBar = '0';
  await BarWallet2.call({ method: 'balance', params: {} })
    .then((n) => {
      walletBar = new BigNumber(n)
        .shiftedBy(-Constants.tokens.bar.decimals)
        .toString();
    })
    .catch((e) => {
  ...
  let walletLp = '0';
  await FooBarLpWallet2.call({ method: 'balance', params: {} })
    .then((n) => {
...
async function dexPairInfo() {
  const balances = await DexPairFooBar.call({
    method: 'getBalances',
    params: {},
  });
  const total_supply = await FooBarLpRoot.call({
    method: 'totalSupply',
    params: {},
  });
  let foo, bar;
...
describe(`DexAccount interact with ${options.pair_contract_name}`, async function () {
  ...
  before('Load contracts', async function () {
    ...
    DexPairFooBar = await locklift.factory.getContract(
      options.pair_contract_name,
    );
    FooRoot = await locklift.factory.getContract(
      'TokenRootUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    BarRoot = await locklift.factory.getContract(
      'TokenRootUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    FooBarLpRoot = await locklift.factory.getContract(
      'TokenRootUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    Account2 = await locklift.factory.getAccount('Wallet');
    DexAccount2 = await locklift.factory.getContract(
      options.account_contract_name,
    );
    FooBarLpWallet2 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    FooWallet2 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    BarWallet2 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    ...
    const pairRoots = await DexPairFooBar.call({
  ...
  describe('Deposits', async function () {
    it('0001 # Add initial liquidity to Foo/Bar', async function () {
      ...
      if (dexPairInfoStart.lp_supply === '0') {
        ...
        const LEFT_AMOUNT = IS_FOO_LEFT
          ? new BigNumber(FOO_DEPOSIT)
              .shiftedBy(Constants.tokens.foo.decimals)
              .toString()
          : new BigNumber(BAR_DEPOSIT)
              .shiftedBy(Constants.tokens.bar.decimals)
              .toString();
        ...
        let LP_REWARD = await expectedDepositLiquidity(
          ...
          options.pair_contract_name,
          IS_FOO_LEFT
            ? [Constants.tokens.foo, Constants.tokens.bar]
            : [Constants.tokens.bar, Constants.tokens.foo],
        ...
        let expectedAccount2Foo = new BigNumber(dexAccount2Start.foo)
          .minus(FOO_DEPOSIT)
          .toString();
        let expectedAccount2Bar = new BigNumber(dexAccount2Start.bar)
          .minus(BAR_DEPOSIT)
          .toString();

        let expectedDexAccount2Lp = new BigNumber(
          dexAccount2Start.lp,
        ).toString();
        ...
        expect(expectedPairFoo).to.equal(
          dexPairInfoEnd.foo,
          'Wrong DexPair FOO',
        );
        expect(expectedPairBar).to.equal(
          dexPairInfoEnd.bar,
          'Wrong DexPair BAR',
        );
    ...
    if (options.pair_contract_name !== 'DexStablePair') {
      it('0005 # Add FOO+BAR liquidity (auto_change=false), surplus BAR must returns', async function () {
        logger.log('#################################################');
        logger.log(
          '# Add FOO+BAR liquidity (auto_change=false), surplus BAR must returns',
        );
        const dexAccount2Start = await dexAccountBalances(DexAccount2);
        const dexPairInfoStart = await dexPairInfo();
        ...
        const LP_REWARD = new BigNumber(expected.step_1_lp_reward)
          ...
          .shiftedBy(-9)
          .toString();
        const BAR_BACK_AMOUNT = new BigNumber(BAR_DEPOSIT).minus(
          new BigNumber(
            IS_FOO_LEFT
              ? expected.step_1_right_deposit
              : expected.step_1_left_deposit,
          ).shiftedBy(-Constants.tokens.bar.decimals),
        );
  ...
  describe('Exchanges', async function () {
    it('0006 # DexAccount#2 exchange FOO to BAR', async function () {
      ...
      logger.log(
        `DexAccount#2 balance start: ${dexAccount2Start.foo} FOO, ${dexAccount2Start.bar} BAR`,
      );
      logger.log(
        `DexPair start: ${dexPairInfoStart.foo} FOO, ${dexPairInfoStart.bar} BAR`,
      );
      ...
      tx = await Account2.runTarget({
        ...
        params: {
          call_id: getRandomNonce(),
          spent_amount: new BigNumber(TOKENS_TO_EXCHANGE)
            .shiftedBy(Constants.tokens.foo.decimals)
            .toString(),
          spent_token_root: FooRoot.address,
      ...
      logger.log(
        `DexAccount#2 balance end: ${dexAccount2End.foo} FOO, ${dexAccount2End.bar} BAR`,
      );
      logger.log(
        `DexPair end: ${dexPairInfoEnd.foo} FOO, ${dexPairInfoEnd.bar} BAR`,
      );
      ...
      expect(expectedPairFoo).to.equal(
        dexPairInfoEnd.foo.toString(),
        'Wrong DEX FOO balance',
      );
      expect(expectedPairBar).to.equal(
        dexPairInfoEnd.bar.toString(),
        'Wrong DEX BAR balance',
      );
      expect(expectedDexAccountFoo).to.equal(
        dexAccount2End.foo.toString(),
        'Wrong DexAccount#2 FOO balance',
      );
      expect(expectedDexAccountBar).to.equal(
        dexAccount2End.bar.toString(),
        'Wrong DexAccount#2 BAR balance',
      );
    });
    ...
    it('0008 # DexAccount#2 exchange FOO to BAR (wrong rate)', async function () {
      ...
      tx = await Account2.runTarget({
        ...
        params: {
          ...
          spent_token_root: FooRoot.address,
          receive_token_root: BarRoot.address,
          expected_amount: new BigNumber(expected.expected_amount)
            .plus(1)
            .toString(),
          send_gas_to: Account2.address,
        },
      ...
      expect(dexPairInfoStart.foo).to.equal(
        dexPairInfoEnd.foo,
        'Wrong DEX FOO balance',
      );
      expect(dexPairInfoStart.bar).to.equal(
        dexPairInfoEnd.bar,
        'Wrong DEX BAR balance',
      );
      expect(dexAccount2Start.foo).to.equal(
        dexAccount2End.foo,
        'Wrong DexAccount#2 FOO balance',
      );
      expect(dexAccount2Start.bar).to.equal(
        dexAccount2End.bar,
        'Wrong DexAccount#2 BAR balance',
      );
    });
