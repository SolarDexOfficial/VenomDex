...
async function account2balances() {
  ...
  const ton = await locklift.utils
    .convertCrystal(await locklift.ton.getBalance(Account2.address), 'ton')
    .toNumber();
  return { foo, bar, lp, ton };
}

async function dexBalances() {
  const foo = await FooVaultWallet.call({ method: 'balance', params: {} }).then(
    (n) => {
  ...
  const bar = await BarVaultWallet.call({ method: 'balance', params: {} }).then(
    (n) => {
      return new BigNumber(n)
        .shiftedBy(-Constants.tokens.bar.decimals)
        .toString();
  ...
  const lp = await FooBarLpVaultWallet.call({
    method: 'balance',
    params: {},
  }).then((n) => {
...
function logBalances(header, dex, account, dexAccount, pair) {
  logger.log(
    `DEX balance ${header}: ${dex.foo} FOO, ${dex.bar} BAR, ${dex.lp} LP`,
  );
  logger.log(
    `DexPair ${header}: ` +
...
describe('DexAccount interact with DexPair', async function () {
  ...
  before('Load contracts', async function () {
    ...
    FooVaultWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    BarVaultWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    FooBarLpVaultWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
  ...
  describe('Withdraw all', async function () {
    it(`Withdraw liquidity from ${left_token.symbol}/${right_token.symbol}`, async function () {
      logger.log('#################################################');
      logger.log(
        `# Withdraw liquidity from ${left_token.symbol}/${right_token.symbol}`,
      );
      ...
      const expected = await DexPairFooBar.call({
        method: 'expectedWithdrawLiquidity',
        params: {
          lp_amount: new BigNumber(account2Start.lp)
            .shiftedBy(Constants.LP_DECIMALS)
            .toString(),
        },
      });
      ...
      const payload = await DexPairFooBar.call({
        method: 'buildWithdrawLiquidityPayload',
        params: {
          id: 0,
      ...
      let tx = await Account2.runTarget({
        ...
        method: 'transfer',
        params: {
          amount: new BigNumber(account2Start.lp)
            .shiftedBy(Constants.LP_DECIMALS)
            .toString(),
      ...
      expect(expectedDexFoo).to.equal(
        dexEnd.foo.toString(),
        'Wrong DEX FOO balance',
      );
      expect(expectedDexBar).to.equal(
        dexEnd.bar.toString(),
        'Wrong DEX BAR balance',
      );
      expect(expectedAccountFoo).to.equal(
        account2End.foo.toString(),
        'Wrong Account#3 FOO balance',
      );
      expect(expectedAccountBar).to.equal(
        account2End.bar.toString(),
        'Wrong Account#3 BAR balance',
      );
      expect(expectedAccountLp).to.equal(
        account2End.lp.toString(),
