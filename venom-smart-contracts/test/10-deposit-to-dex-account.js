...
const loadWallets = async (data) => {
  ...
  data.vaultWalletBalance = new BigNumber(
    await data.vaultWallet.call({ method: 'balance' }),
  )
  ...
  data.accountWalletBalance = new BigNumber(
    await data.accountWallet.call({ method: 'balance' }),
  )
    .shiftedBy(-tokenData.decimals)
    .toString();
  data.dexAccountWallet = await locklift.factory.getContract(
    'TokenWalletUpgradeable',
    TOKEN_CONTRACTS_PATH,
  );
  ...
  data.dexAccountVirtualBalance = new BigNumber(accountNWalletData.balance)
    .shiftedBy(-tokenData.decimals)
    .toString();
  data.dexAccountWalletBalance = (
    await data.dexAccountWallet.call({ method: 'balance' })
  )
    .shiftedBy(-tokenData.decimals)
    .toString();
};
