...
options.roots = options.roots
  ...
  : ['foo', 'bar', 'qwe'];
options.amounts = options.amounts
  ? JSON.parse(options.amounts)
...
async function dexPoolInfo() {
  ...
  const total_supply = await poolLpRoot.call({
    method: 'totalSupply',
    params: {},
  });
  const tokenBalances = [];
  for (let i = 0; i < N_COINS; i++) {
    tokenBalances.push(
      new BigNumber(balances.balances[i])
        .shiftedBy(-tokens[i].decimals)
        .toString(),
    );
  }
...
describe('DexAccount interact with DexPool', async function () {
  ...
  before('Load contracts', async function () {
    ...
    let roots = (await DexPool.call({ method: 'getTokenRoots', params: {} }))
      .roots;
  ...
  describe('Deposit', async function () {
    it(`Deposit liquidity to ${poolName}`, async function () {
      ...
      for (let i = 0; i < N_COINS; i++) {
        logs +=
          dexAccount2Start.accountBalances[i] + ' ' + tokens[i].symbol + ', ';
      }
      logs +=
        dexAccount2Start.lp +
        ' LP, ' +
        dexAccount2Start.walletLp +
        ' LP (wallet)';
      logger.log(logs);
      ...
      for (let i = 0; i < N_COINS; i++) {
        logs +=
          dexPoolInfoStart.token_balances[i] + ' ' + tokens[i].symbol + ', ';
      }
      logs +=
        `LP SUPPLY (PLAN): ${dexPoolInfoStart.lp_supply || '0'} LP, ` +
      ...
      const amounts = [];
      for (let i = 0; i < N_COINS; i++) {
        const amount = new BigNumber(options.amounts[i])
          .shiftedBy(tokens[i].decimals)
          .toString();
        amounts.push(amount);
        operations.push({
      ...
      for (let i = 0; i < N_COINS; i++) {
        logs +=
          dexAccount2End.accountBalances[i] + ' ' + tokens[i].symbol + ', ';
      }
      logs +=
        dexAccount2End.lp + ' LP, ' + dexAccount2End.walletLp + ' LP (wallet)';
      logger.log(logs);

      logs = `DexPool end: `;
      for (let i = 0; i < N_COINS; i++) {
        logs +=
          dexPoolInfoEnd.token_balances[i] + ' ' + tokens[i].symbol + ', ';
      }
      logs +=
        `LP SUPPLY (PLAN): ${dexPoolInfoEnd.lp_supply || '0'} LP, ` +
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(expectedAccount2Amount[i]).to.equal(
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(expectedPoolAmount[i]).to.equal(
          dexPoolInfoEnd.token_balances[i],
